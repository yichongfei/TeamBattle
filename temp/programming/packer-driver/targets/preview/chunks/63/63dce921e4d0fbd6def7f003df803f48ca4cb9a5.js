    ...
    execute: function () {
      ...
      _export("MovementComponent", MovementComponent = (_dec = ccclass('MovementComponent'), _dec2 = property(_crd && CharacterStats === void 0 ? (_reportPossibleCrUseOfCharacterStats({
        ...
        calculateSteeringForces(deltaTime) {
          ...
          var maxSpeed = this.stats.moveSpeed;
          var arriveForce = Vec3.ZERO;
          ...
          var rawSeparationForce = this.separate(this._squadMates, maxSpeed);
          var weightedSeparationForce = rawSeparationForce.multiplyScalar(this.separationWeight);
        ...
        applySteeringToVelocity(deltaTime) {
          ...
          var acceleration = this._steeringForce.clone().multiplyScalar(deltaTime);
        ...
        updatePosition(deltaTime) {
          ...
          var moveDelta = this._velocity.clone().multiplyScalar(deltaTime); // 更新世界坐标
        ...
        updateOrientation() {
          ...
          var speedThresholdSq = 1.0 * 1.0; // 速度平方阈值，可以调整
        ...
        arrive(target, maxSpeed) {
          ...
          var steer = desired.subtract(this._velocity);
          return steer;
        }
        ...
        separate(neighbors, maxSpeed) {
          ...
          if (neighbors) {
            for (var otherComp of neighbors) {
              if (!otherComp || !otherComp.isValid || !otherComp.node) continue;
              var otherNode = otherComp.node;
              var dist = Vec3.distance(this.node.worldPosition, otherNode.worldPosition);

              if (dist > 0 && dist < desiredSeparation) {
                var diff = this.node.worldPosition.clone().subtract(otherNode.worldPosition);
                diff.normalize();
          ...
          var finalSteer = Vec3.ZERO.clone();
