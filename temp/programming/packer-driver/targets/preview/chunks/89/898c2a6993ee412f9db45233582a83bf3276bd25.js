    ...
    execute: function () {
      ...
      _export("AIComponent", AIComponent = (_dec = ccclass('AIComponent'), _dec2 = property(_crd && CharacterStats === void 0 ? (_reportPossibleCrUseOfCharacterStats({
        ...
        onLoad() {
          ...
          var squadManager = (_crd && PlayerSquadManager === void 0 ? (_reportPossibleCrUseOfPlayerSquadManager({
            error: Error()
        ...
        runSimplePlaceholderLogic(deltaTime) {
          ...
          var targetNode = target == null ? void 0 : target.getNode();
          var isSelfAlive = (_this$health$isAlive = (_this$health = this.health) == null ? void 0 : _this$health.isAlive()) != null ? _this$health$isAlive : false;

          if (target && isSelfAlive && targetNode && targetNode.isValid) {
            var bossPosition = target.getPosition();
            var distanceToBoss = Vec3.distance(this.node.worldPosition, bossPosition);
            var attackRange = this.stats.attackRange; // 定义一个非常小的缓冲距离，防止在边缘抖动

            var attackRangeBuffer = 5; // --- 移动目标点现在直接是 Boss 位置 (考虑区域限制) ---

            var squadMgr = (_crd && PlayerSquadManager === void 0 ? (_reportPossibleCrUseOfPlayerSquadManager({
              error: Error()
            }), PlayerSquadManager) : PlayerSquadManager).instance;
            var myIndex = squadMgr ? squadMgr.getSquadIndex(this.node) : 0;
            var squadSize = Math.max(1, squadMgr ? squadMgr.getTotalSquadSize() : 1); // 站位半径：比攻击距离略小，让武器打得到

            var ringRadius = this.stats.attackRange * 0.8; // 均分 360°，每人一个角度槽

            var angle = Math.PI * 2 / squadSize * myIndex; // 计算在环上的目标点

            var finalMoveTarget = bossPosition.clone();
            finalMoveTarget.x += Math.cos(angle) * ringRadius;
            finalMoveTarget.y += Math.sin(angle) * ringRadius;
            var distFromCenter = Vec3.distance(finalMoveTarget, this.stageCenter);

            if (this.maxDistanceFromCenter > 0 && distFromCenter > this.maxDistanceFromCenter) {
              var directionFromCenter = finalMoveTarget.clone().subtract(this.stageCenter).normalize();
            ...
            if (distanceToBoss <= attackRange - attackRangeBuffer) {
              ...
              var directionToTargetX = bossPosition.x - this.node.worldPosition.x;
              var currentScaleX = Math.abs(this.node.scale.x);
