    ...
    execute: function () {
      ...
      _export("CameraFollow", CameraFollow = (_dec = ccclass('CameraFollow'), _dec2 = property({
        ...
        lateUpdate(deltaTime) {
          ...
          var alivePlayerNodes = this.playerSquadManager.getAlivePlayerCharacters();
          var currentBoss = null;

          if (this.bossManager) {
            var bosses = this.bossManager.getActiveBosses(); // 假设 BossManager 已经过滤了无效/死亡 Boss
            ...
            if (bosses && bosses.length > 0) {
              ...
              var bossHealth = (_currentBoss = currentBoss) == null ? void 0 : _currentBoss.getComponent(_crd && HealthComponent === void 0 ? (_reportPossibleCrUseOfHealthComponent({
                error: Error()
          ...
          var squadCenterPosition = new Vec3(this.node.worldPosition.x, this.node.worldPosition.y, 0); // Fallback to current camera XY
          ...
          var targetCameraPos = new Vec3(squadCenterPosition.x, squadCenterPosition.y, this.initialZ);
          ...
          var localTargetPos = new Vec3();

          if (this.node.parent) {
            // 确保摄像机有父节点 (通常是 Scene)
            var parentWorldMatrix = this.node.parent.worldMatrix;
            var invParentWorldMatrix = new math.Mat4();
        ...
        calculateDesiredOrthoHeight(bounds) {
          // 考虑填充
          var paddedWidth = bounds.width * (1 + this.padding);
          var paddedHeight = bounds.height * (1 + this.padding); // 获取屏幕宽高比

          var screenSize = view.getVisibleSize();
          var aspectRatio = screenSize.width / screenSize.height; // 计算所需的 orthoHeight

          var orthoHeightBasedOnWidth = paddedWidth / aspectRatio / 2;
          var orthoHeightBasedOnHeight = paddedHeight / 2; // 取两者中较大的值，确保都能容纳
          // 同时确保 orthoHeight 不会是 NaN 或 Infinity (例如当 bounds.width/height 为 0 时)

          var height = Math.max(orthoHeightBasedOnWidth, orthoHeightBasedOnHeight);
