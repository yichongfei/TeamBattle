    ...
    execute: function () {
      ...
      _export("CharacterControl", CharacterControl = (_dec = ccclass('CharacterControl'), _dec2 = property({
        ...
        updatePosition(deltaTime) {
          // 1) 先算本帧移动的世界量
          const moveDelta = this.velocity.clone().multiplyScalar(deltaTime); // 2) 在世界坐标系下更新位置
        ...
        updateAnimationAndOrientation() {
          ...
          const currentAnim = (_this$spineAnimation = this.spineAnimation) == null ? void 0 : _this$spineAnimation.animation;
        ...
        arrive(target) {
          ...
          const distance = desired.length(); // 非常接近时直接设置位置并停止
          ...
          const effectiveSlowingRadius = this.slowingRadius; // 在减速半径内调整期望速度
          ...
          const steer = desired.subtract(this.velocity); // 如果非常接近目标，增加转向力的强度以确保能到达
        ...
        separate(neighbors) {
          ...
          for (const other of neighbors) {
            ...
            const distance = Vec3.distance(this.node.position, other.node.position); // 检查其他角色是否是邻居（在半径内）
