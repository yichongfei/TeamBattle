    ...
    execute: function () {
      ...
      _export("TargetingComponent", TargetingComponent = (_dec = ccclass('TargetingComponent'), _dec2 = property(_crd && RoleComponent === void 0 ? (_reportPossibleCrUseOfRoleComponent({
        ...
        findNearestEnemy() {
          ...
          var minDistanceSq = this.aggroRadius > 0 ? this.aggroRadius * this.aggroRadius : Infinity;

          for (var bossNode of bosses) {
            if (!bossNode || !bossNode.isValid) {
              continue;
            }

            var healthComp = bossNode.getComponent(_crd && HealthComponent === void 0 ? (_reportPossibleCrUseOfHealthComponent({
              error: Error()
            ...
            var targetable = healthComp;
            var distanceSq = Vec3.squaredDistance(this.node.worldPosition, targetable.getPosition());
        ...
        isTargetValid(target) {
          ...
          var healthComp = target.getNode().getComponent(_crd && HealthComponent === void 0 ? (_reportPossibleCrUseOfHealthComponent({
            error: Error()
          }), HealthComponent) : HealthComponent);
