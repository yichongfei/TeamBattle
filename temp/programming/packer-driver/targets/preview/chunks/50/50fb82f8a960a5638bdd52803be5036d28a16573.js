    ...
    execute: function () {
      ...
      _export("DamageControl", DamageControl = (_dec = ccclass('DamageControl'), _dec2 = property(_crd && PopUpLabel === void 0 ? (_reportPossibleCrUseOfPopUpLabel({
        ...
        handleDamageEvent(eventData) {
          ...
          var damageStr = Math.round(eventData.damage).toString();
        ...
        handleHealEvent(eventData) {
          ...
          var healStr = Math.round(eventData.healAmount).toString(); // 治疗默认不暴击，类型为 Heal
        ...
        showNumberPopup(targetNode, numberStr, isCrit, popupType) {
          ...
          var uiTransTarget = targetNode.getComponent(UITransform);
          if (!uiTransTarget) return;
          var headOffset = uiTransTarget.height / 2;
          var worldPos = uiTransTarget.convertToWorldSpaceAR(new Vec3(0, 0, 0));
          var uiTransPopup = this.damagePopup.node.getComponent(UITransform);
          if (!uiTransPopup) return;
          var localPos = uiTransPopup.convertToNodeSpaceAR(worldPos);
          ...
          anim.ease = "backOut";
          var displayColor = this.getPopupTypeColor(popupType);
          var initialHorizontalOffset = (Math.random() * 2 - 1) * 60;
          var initialVerticalOffset = (Math.random() * 2 - 1) * 10;
          ...
          var endHorizontalOffset = initialHorizontalOffset + (Math.random() * 2 - 1) * 30;
