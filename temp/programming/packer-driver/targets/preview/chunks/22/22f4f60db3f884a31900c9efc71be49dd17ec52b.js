System.register(["cc"], function (_export, _context) {
  ...
  var _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, _decorator, Component, log, warn, _dec, _class, _crd, ccclass, property, SkillComponent;
  ...
  return {
    ...
    execute: function () {
      ...
      _cclegacy._RF.push({}, "e38da2dop5BvrnWlzdqk3rn", "SkillComponent", undefined);
      ...
      __checkObsolete__(['_decorator', 'Component', 'log', 'warn']);

      ({
        ccclass,
        property
      } = _decorator); // 内部用于追踪技能状态的数据结构

      /**
       * 管理角色拥有的技能列表、冷却时间和使用请求。
       * AIComponent 会查询此组件以决定是否使用技能，并请求使用。
       */
      _export("SkillComponent", SkillComponent = (_dec = ccclass('SkillComponent'), _dec(_class = class SkillComponent extends Component {
        constructor() {
          super(...arguments);
        ...
        update(deltaTime) {
          // 更新所有技能的冷却时间
          for (var info of this._skills.values()) {
            if (!info.isReady) {
              // TODO: 应用冷却缩减 (stats.cooldownReduction)
              info.cooldownTimer -= deltaTime;

              if (info.cooldownTimer <= 0) {
                info.isReady = true; // log(`[${this.node.name}] Skill [${info.skill.id}] is ready.`); // 暂时注释，因为 SkillBase 未定义
                // 发出技能就绪事件?
        ...
        isSkillReady(skillId) {
          var info = this._skills.get(skillId);

          if (!info) return false; // TODO: 后续可以检查资源 (MP), 施法状态等
        ...
        tryUseSkill(skillId) {
          ...
          log("[" + this.node.name + "] Using skill [" + skillId + "]..."); // a. 消耗资源
          ...
          this.scheduleOnce(() => {
            this.executeSkillEffectPlaceholder(skillId);
          }, 0.1); // 临时占位
        ...
        executeSkillEffectPlaceholder(skillId) {
          log("Skill [" + skillId + "] effect triggered (Placeholder)."); // TODO: 实现真正的技能效果逻辑
      ...
      }) || _class)); // --- 辅助类型/接口 (未来可能移到 skills/ 目录下) ---
...
//# sourceMappingURL=22f4f60db3f884a31900c9efc71be49dd17ec52b.js.map