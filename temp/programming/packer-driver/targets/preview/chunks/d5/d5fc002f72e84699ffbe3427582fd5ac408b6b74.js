    ...
    execute: function () {
      ...
      _export("HealthComponent", HealthComponent = (_dec = ccclass('HealthComponent'), _dec2 = property(_crd && CharacterStats === void 0 ? (_reportPossibleCrUseOfCharacterStats({
        ...
        applyDamage(potentialDamage, type, isCrit, source) {
          ...
          var finalDamage = potentialDamage;

          if (type === (_crd && AttackType === void 0 ? (_reportPossibleCrUseOfAttackType({
            error: Error()
          }), AttackType) : AttackType).PHYSICAL) {
            var armor = targetStats.getCurrentArmor();
            var reductionFactor = 500;
            var damageReduction = armor / (armor + reductionFactor);
          ...
          if (finalDamage > 0 && source && source.isValid) {
            var attackerStats = source.getComponent(_crd && CharacterStats === void 0 ? (_reportPossibleCrUseOfCharacterStats({
              error: Error()
            }), CharacterStats) : CharacterStats);
            var attackerHealth = source.getComponent(HealthComponent);

            if (attackerStats && attackerHealth) {
              var lifestealRatio = attackerStats.getCurrentLifesteal();

              if (lifestealRatio > 0) {
                var lifestealAmount = finalDamage * lifestealRatio;
        ...
        applyHeal(amount, source) {
          ...
          var actualHealed = this._currentHealth - oldHealth;
